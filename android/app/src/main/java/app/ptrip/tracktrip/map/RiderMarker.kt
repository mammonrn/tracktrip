 ...
 *
 * The pale outline is not decoration: against a park, a lake or a motorway
 * shield, an unoutlined pin in a bright hue loses its edge. It is the app's
 * own background colour, which is what makes the pins read as belonging to
 * this app rather than to the cartography under them.
 */
object RiderMarker {
    ...
    private fun drawPin(color: Int, label: String, resources: Resources): Drawable {
        ...
        // The pin's ring, the hole in its middle, and the plate under its
        // name: all the app's ground colour, so a pin is one object.
        val outline = AppBackground.toArgb()
        ...
        // A hole in the middle, so two riders in similar hues are still two
        // rings rather than two blobs.
        canvas.drawCircle(
            centre,
            radius,
            radius * 0.30f,
        ...
        // The name, on a plate of the same colour. Over a busy tile the plate
        // is the only thing that keeps coloured text readable, and it is what
        // stops two riders' names running into each other when their pins are
        // close.
